...
Model::Model(std::string name, int vao, int vertexCount, int shaderProgram)
	: name(name), vao(vao), vertexCount(vertexCount), shader(shaderProgram), position(0.0f, 0.0f, 0.0f), scaling(1.0f, 1.0f, 1.0f), rotationAxis(0.0f, 0.0f, 1.0f),
	rotationAngleInDegrees(0.0f), velocity(0.0f), angularAxis(0, 0, 1), angularVelocityInDegrees(0.0f)
{
}
...
glm::mat4 Model::GetWorldMatrix() const
{
	...
	mat4 r = rotate(mat4(1.0f), glm::radians(rotationAngleInDegrees), rotationAxis);
...
GLuint Model::createVAO(int& vertexCount, std::vector<glm::vec3> positions, std::vector<glm::vec3> normals, std::vector<glm::vec2> uvs)
{
	GLuint vao;
	GLuint vbo[3];
	...
	glGenBuffers(3, vbo); //4, for positions, normals, colors, uvs
	...
	// Setup Colors
//	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]); //colors
...
//	glEnableVertexAttribArray(1);
	...
	// Setup UVs
	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]); //UVs
